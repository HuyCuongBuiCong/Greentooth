...
public class BluetoothChatService {
	...
	private synchronized void setState(int state) {
		...
		mHandler.obtainMessage(Const.MESSAGE_STATE_CHANGE, state, -1)
				.sendToTarget();
	}
	...
	public synchronized void connect(BluetoothDevice device) {
		...
		// Cancel any thread attempting to make a connection
		if (mState == Const.STATE_CONNECTING) {
			if (mConnectThread != null) {
				mConnectThread.cancel();
				mConnectThread = null;
			}
	...
	public void write(byte[] out) {
		...
		synchronized (this) {
			if (mState != Const.STATE_CONNECTED)
				return;
			r = mConnectedThread;
		}
	...
	private class AcceptThread extends Thread {
		...
		public void run() {
			...
			while (mState != Const.STATE_CONNECTED) {
				try {
	...
	private class ConnectedThread extends Thread {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					mHandler.obtainMessage(Const.MESSAGE_READ, bytes,
		...
		public void write(byte[] buffer) {
			try {
				...
				mHandler.obtainMessage(Const.MESSAGE_WRITE, -1, -1,
						buffer).sendToTarget();
			} catch (IOException e) {
